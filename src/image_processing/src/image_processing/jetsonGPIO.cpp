...
int gpioExport ( jetsonGPIO gpio )
{
    ...
    string prePath = SYSFS_GPIO_DIR;
    string path = prePath + "/export";
...
int gpioSetDirection ( jetsonGPIO gpio, unsigned int out_flag )
{
    ...
    //string path = prePath + "/export";
    //const char * c = path.c_str();
    ofstream file(commandBuffer);
    cout << "path" << commandBuffer <<endl;

    if(file.is_open())
{
    cout << "direction file is open "<< endl;

    if (out_flag) {
           snprintf(commandBuffer, sizeof(commandBuffer), "out");
           file<<commandBuffer;
           cout << "set pin" << gpio << " as " << commandBuffer  <<endl; 
    }
    else {
           snprintf(commandBuffer, sizeof(commandBuffer), "in");
	file << commandBuffer;
	cout << "set pin" << gpio << " as " << commandBuffer <<endl;
    ...
    if (!infile.is_open()) 
{
	...
	cout << "reading character from commandBuffer" << endl;

	infile >> ch;
	cout << ch << endl;

   // if (read(&ch,1) != 1) && file.is_open() ) {
   //     perror("gpioGetValue") ;
   //     return 0 ;
   //  }
